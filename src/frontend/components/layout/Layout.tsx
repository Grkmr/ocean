...
function Layout({ pageName, darkMode, children }: LayoutProps) {
	...
	const displayedLoadingText = useMemo(
		() => (loadingText !== true ? loadingText : "Loading"),
		[loadingText],
	);
			...
			<main className={inter.className}>
								...
								<Alert variant="danger" dismissible>
									...
									{typeof errorMessage === "string" &&
										errorMessage.split("\n").map((line, i) => {
											...
											const indent =
												4 * (indentPrefix.match(/\t/g) || []).length +
												(indentPrefix.match(/ /g) || []).length;
											return (
												<p
													key={i}
													className="mb-0"
													style={{ marginLeft: `${0.75 * indent}rem` }}
												>
													{line.trim()}
												</p>
											);
				...
				{!!toasts.length && (
					<ToastContainer
						...
						style={{ zIndex: 1 }}
					>
						{toasts.slice(-maxNumToasts).map((props, i) => (
							<Toast key={i} {...props} />
						))}
					</ToastContainer>
				)}
				...
				{!!(isLoading && displayedLoadingText) && (
					...
					>
						...
						>
							...
							{!displayedLoadingText.endsWith("...") && (
								<span className="ps-1 position-absolute">...</span>
							)}
						</p>
					</div>
				)}
...
export const Sidebar: React.FC<HTMLProps<HTMLDivElement>> = ({
	children,
	...props
}) => {
