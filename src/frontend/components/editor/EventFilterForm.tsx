...
const EditorFilterForm = () => {
	const { register, handleSubmit, control, reset } = useForm<EventFilter>({
		defaultValues: {
			...
			{events && (
				<>
					<Table>
						...
						<tbody>
							{events.map(({ id, timestamp, attr, activity }) => (
								<tr>
									<td>{id}</td>
									<td>{activity}</td>
									<td>{timestamp}</td>
									{events[0] &&
										Object.values(attr).map((data) => (
