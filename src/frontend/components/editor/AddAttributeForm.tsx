...
import { upsertAttributes, useOcelInfo } from "@/hooks/api";
...
const AttributeUpsertForm = () => {
  ...
  const attributeList = useMemo(
    () => [...new Set([
      ...(target === "events" ? OCELEventOptions : OCELObjectOptions),
      ...(target === "events"
        ? Object.values(ocelInfo?.event_summaries ?? {}).flatMap((attributes) =>
          attributes.map(({ attribute }) => attribute)
        )
        : []),
      ...(target === "objects" && ocelInfo?.object_summaries
        ? ocelInfo?.object_summaries.flatMap(({ attributes }) => attributes.map(({ attribute }) => attribute))
        : []),
    ])],
    [target, ocelInfo?.object_summaries, ocelInfo?.event_summaries],
  );
              ...
              <Row key={field.id} className="mb-2 align-items-center">
                <Col>
                  <Form.Select {...register(`mergeFields.${idx}.ocelField`)}>
                    <option value="" hidden>{`${target === "events" ? "Event" : "Object"} field`}</option>
                    {attributeList.map((f) => (
                ...
                <Col>
                  <Form.Select {...register(`mergeFields.${idx}.csvField`)}>
                    <option value="" hidden >CSV field</option>
                    {csvHeaders.map((f) => (
                      <option key={f} value={f}>
                        {f}
                      </option>
