...
const ClimatiqPage: React.FC<PageProps> = ({}) => {
	...
	>();
	const [climatiqRequest, setClimatiqRequest] = useState<
		ClimatiqSearchRequest | undefined
	>(initialClimatiqRequest);
	const [page, setPage] = useState(1);
	const [querySearchValue, setQuerySearchValue] = useState<string>(
		_.get(initialClimatiqRequest, "query") ?? "",
	);
	const [waitingForClimatiqResponse, setWaitingForClimatiqResponse] =
		useState<boolean>(true);

	const [selectedEmissionFactorGroup, setSelectedEmissionFactorGroup] =
		useState<ClimatiqEmissionFactor[] | undefined>(undefined);
	...
	useEffect(() => {
		...
		async function effect() {
			...
			const response = await climatiqSearchRequest(
				climatiqRequest,
				climatiqConfig,
			);
	...
	useEffect(() => {
		if (!climatiqRequest || !climatiqResponse) {
			console.warn(
				"Cannot change page, either climatiqRequest or climatiqResponse is not set.",
			);
			return;
		}
		if (page < 1 || page > climatiqResponse.lastPage) {
			console.warn(
				`Cannot change page, the page number is invalid. Returning to page 1. (allowed 1-${climatiqResponse.lastPage}, got ${page})`,
			);
			setPage(1);
			return;
		}
	...
	const factorsGroupedByActivity = useMemo(() => {
		...
		return Object.values(
			_.groupBy(climatiqResponse.results, (ef) => ef.activityId ?? ef.id),
					...
					<>
						...
						>
							{[20, 50, 100].map((resultsPerPage, i) => (
								<option key={i} value={resultsPerPage}>
									{resultsPerPage}
								</option>
							))}
						</Form.Select>
						<span style={{ whiteSpace: "nowrap" }}>
							of {climatiqResponse.totalResults}
						</span>
					</>
				)}
				{climatiqResponse.lastPage == 1 && (
					<span style={{ whiteSpace: "nowrap" }}>
						{climatiqResponse.totalResults} results
					</span>
				)}
			</div>

			<Pagination
				max={climatiqResponse.lastPage}
				current={page}
				setCurrent={setPage}
				className="ms-auto"
			/>
	...
	return (
		<>
						...
						<ClimatiqFilters>
							...
							{showResetFilters && (
								<Button
									...
									onClick={() => setClimatiqRequest(initialClimatiqRequest)}
								>
									Reset filters
								</Button>
							)}
				...
				<Row>
					<Col>
						...
						<Table size="sm" striped bordered hover>
							...
							<tbody>
								{(!waitingForClimatiqResponse && factorsGroupedByActivity
									? factorsGroupedByActivity
									: _.range(climatiqRequest.resultsPerPage).map(
											(i) => undefined,
										)
								).map((group, i) => {
									...
									const categories = _.uniq(
										(group ?? []).map((ef) => ef.category),
									);
									const sources = _.uniq((group ?? []).map((ef) => ef.source)); // TODO objects
									const regions = _.uniqBy(
										(group ?? []).map((ef) => ef.region),
										(region) => region.id,
									);
									const unitTypes = _.uniq(
										(group ?? []).map((ef) => ef.unitType),
									);
			...
			{climatiqRequest &&
				climatiqResponse &&
				selectedEmissionFactorGroup &&
				selectedEmissionFactorGroup.length > 0 && (
