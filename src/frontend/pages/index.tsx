...
import {
	...
	AggModeSelector,
	AggProgressBar,
} from "@/components/common/AggMode";
...
const StartPage: React.FC<PageProps> = ({ apiWrapper }) => {
	...
	const { objectTypeColors, setObjectTypeColors } =
		useOceanStore.useState.objectTypeColors();
	...
	const objectSampleAttrs = useMemo(() => {
		if (!objectSample) return undefined;
		return _.uniq(
			objectSample?.map((obj) => Object.keys(obj.attr)).flat(1),
		).toSorted();
	}, [objectSample]);

	const [eventSample, setEventSample] = useState<OcelEvent[]>();
	const eventSampleAttrs = useMemo(() => {
		if (!eventSample) return undefined;
		return _.uniq(
			eventSample?.map((ev) => Object.keys(ev.attr)).flat(1),
		).toSorted();
	}, [eventSample]);
	...
	const handleUpload = useCallback(async () => {
		...
		const { ocelBlob, ocelFileName, appState, error } =
			await processOcelFile(selectedFile);
	...
	const handleSampleEvents = useCallback(async () => {
		...
		const data = await apiWrapper(
			() =>
				Api.sampleEventsSampleEventsGet({
	...
	const handleSampleObjects = useCallback(async () => {
		...
		const data = await apiWrapper(
			() =>
				Api.sampleObjectsSampleObjectsGet({
					oceanSessionId: session,
				}),
			{
	...
	const handleDiscoverPetriNet = useCallback(async () => {
		...
		if (
			!objectTypes ||
			!objectTypes.length ||
			!objectTypes.every((ot) => ocel.objectTypes.includes(ot))
		) {
			return false;
		}
		const data = await apiWrapper(
	...
	const eventAttributes =
		ocel?.attributes.filter((attr) => isEventAttribute(attr)) ?? [];
	const objectAttributes =
		ocel?.attributes.filter((attr) => isObjectAttribute(attr)) ?? [];
	...
	const { emissionAttributes, setEmissionAttributes } =
		useOceanStore.useState.emissionAttributes();
	const {
		isEmissionAttributeSelectionOpen,
		setIsEmissionAttributeSelectionOpen,
	} = useOceanStore.useState.isEmissionAttributeSelectionOpen();
	const [editingEmissionAttributes, setEditingEmissionAttributes] = useState(
		emissionAttributes ?? [],
	);
	...
	const [activityEmissionsAggMode, setActivityEmissionsAggMode] =
		useState<AggMode>(aggModes.mean);
				...
				<>
						...
						<Col lg={6} className="mb-3">
							...
							<ButtonToolbar className="mt-2">
								<Button
									variant="primary"
									onClick={() => {
										handleDiscoverPetriNet();
									}}
									disabled={
										ocpn && showOcpn && handlingUnits
											? _.isEqual(
													ocpn.objectTypes.toSorted(),
													handlingUnits.toSorted(),
												)
											: false
									}
								>
							...
							<>
								<Col lg={6} className="mb-3">
									...
									<p className="mb-2 small">
										Select which object types are handling units (HUs) and which
										are process resources.
									</p>
									<div className="mb-2">
										<ObjectTypeManager
											minNumCols={2}
					...
					<Row>
						<Col>
							...
							<Tabs
								id="attribute-manager-tabs"
								defaultActiveKey={eventAttributes.length ? "eattrs" : "oattrs"}
							>
								<Tab
									title={`Event attributes (${eventAttributes.length})`}
									eventKey="eattrs"
									disabled={!eventAttributes.length}
								>
									<AttributeManager
										target="event"
								...
								<Tab
									title={`Object attributes (${objectAttributes.length})`}
									eventKey="oattrs"
									disabled={!objectAttributes.length}
								>
									<AttributeManager
										target="object"
								...
								{!!eventAttributes.length && !!objectAttributes.length && (
									<Tab
										title={`All (${ocel.attributes.length})`}
										eventKey="all"
										disabled={!ocel.attributes.length}
									>
										<AttributeManager
											target="both"
											attributes={ocel.attributes}
										/>
									</Tab>
								)}
							</Tabs>
					...
					<Row>
						<Col
							md={emissions && emissions.activityEmissions ? 8 : 6}
							lg={6}
							className="mb-3"
						>
							<h4>Activities</h4>
							<Table striped size="sm">
								...
								<tbody>
									{_.sortBy(
										Object.entries(ocel.activityCounts),
										([_act, count]) => -count,
									).map(([activity, count], i) => (
										<tr key={i}>
											<td>
												<ActivityWithIcon activity={activity} />
											</td>
											{emissions && emissions.activityEmissions && (
												<>
													<td
						...
						<Col
							md={emissions && emissions.activityEmissions ? 4 : 6}
							lg={6}
							className="mb-3"
						>
							<h4>Object types</h4>
							<Table striped size="sm">
								...
								<tbody>
									{_.sortBy(
										Object.entries(ocel.objectTypeCounts),
										([_ot, count]) => -count,
									).map(([objectType, count], i) => {
										// const iconColor = ((objectTypeColors ?? {})[objectType])?.hex() ?? "var(--bs-secondary)"

										return (
											<tr key={i}>
												<td>
													<ObjectTypeWithIcon objectType={objectType} />
												</td>
												<td
													className="align-middle"
													style={{ width: "30%", minWidth: "80px" }}
												>
													<ProgressBar
														...
														min={0.5}
														rounded={false}
														color={_.get(objectTypeColors ?? {}, objectType)}
														variant="secondary"
													/>
												</td>
											</tr>
					...
					<Modal
						...
						size="xl"
						fullscreen="lg-down"
						onHide={() => setObjectSample(undefined)}
					>
						...
						<Modal.Footer>
							<ButtonToolbar>
								...
								<Button
									variant="secondary"
									onClick={() => setObjectSample(undefined)}
								>
									Close
								</Button>
							</ButtonToolbar>
					...
					<Modal
						...
						size="xl"
						fullscreen="lg-down"
						onHide={() => setEventSample(undefined)}
					>
						<Modal.Header closeButton>
						...
						<Modal.Footer>
							<ButtonToolbar>
								...
								<Button
									variant="secondary"
									onClick={() => setEventSample(undefined)}
								>
									Close
								</Button>
							</ButtonToolbar>
					...
					<Modal
						show={isEmissionAttributeSelectionOpen}
						size="xl"
						fullscreen="md-down"
						onHide={() => setIsEmissionAttributeSelectionOpen(false)}
					>
						<Modal.Header closeButton>
						...
						<Modal.Body>
							<Alert variant="light" dismissible>
								...
								Only numeric event attributes are shown. OCEAn currently does
								not support importing object emissions. To use an object
								emission attribute, create an emission rule using that
								attribute.
							</Alert>
							<AttributeManager
								target="event"
				...
				<>
					...
					<div className="mb-5">
						<Form.Group controlId="formFile" className="mb-3">
							<Form.Label>
								Select an event log (<code>.sqlite</code> OCEL 2.0 format) from
								your local disk
							</Form.Label>
							<Form.Control
								type="file"
											...
											<div key={i}>
												...
												<a
													...
													onClick={async () => {
														await apiWrapper(
															() =>
																Api.importDefaultOcelImportDefaultGet({
																	key: key,
																	version: version,
																}),
															{
