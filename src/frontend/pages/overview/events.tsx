...
import { useOcelInfo, usePaginatedEvents, usePaginatedObjects } from "@/hooks/api"
...
const EventOverview: React.FC<PageProps> = ({ apiWrapper }) => {
  ...
  const { data: events } = usePaginatedEvents({ filter: { event_attributes: null, ...(view === "events" && viewTab && { activity_names: [viewTab] }) } })
  const { data: objects } = usePaginatedObjects({ filter: { ...(view === "objects" && viewTab && { object_types: [viewTab] }) } })
  ...
  return (
    <>
      ...
      {
        (((view === "events" && events) || (view === "objects" && objects)) && info) && <>
          {view === "events" ? <>
            <Tabs
              activeKey={viewTab ?? info.activities[0].activity!}
              onSelect={(k) => { setViewTab(k); handlePageChange(1) }}
              className="mb-3"
            >
              {info.activities.map(({ activity }) => <Tab eventKey={activity} title={activity} />)}
            </Tabs>
            ...
            <Tabs
              ...
              className="mb-3"
            >
              {info.object_summaries.map(({ object_type }) => <Tab eventKey={object_type} title={object_type} />)}
            </Tabs>
