...
const EmissionsPage: React.FC<PageProps> = ({ apiWrapper }) => {
	...
	const [editingEmissionRules, setEditingEmissionRules] =
		useState<EditingEmissionRule[]>(emissionRules);
	...
	useEffect(() => {
		const nextEmissionRules = editingEmissionRules.filter(
			(er) => !isEmissionRuleEmpty(er),
		);
	...
	const nextEmissionRuleIndex = useMemo(() => {
		if (!editingEmissionRules.length) return 0;
	...
	const handleApplyEmissionRules = useCallback(async () => {
		...
		apiWrapper(
			() =>
				Api.computeEmissionsComputeEmissionsPost({
					oceanSessionId: session,
					requestBody: {
						rules: emissionRules.filter(
							(ec) => !isEmissionRuleEmpty(ec),
						) as unknown as (EventEmissionRule_Input | E2OEmissionRule_Input)[],
					},
				}),
			{
		...
		<>
									...
									<EmissionRuleFormWrapper
										...
										saveRule={async (
											...
											if (er === null) {
												...
												if (
													!prev ||
													isEmissionRuleEmpty(prev) ||
													(await confirm(
														"Dou you want to delete the emission rule?",
														{
															confirmText: "Delete",
					...
					<ButtonToolbar>
						...
						{!!editingEmissionRules.length && (
							<Button
								...
								disabled={
									editingEmissionRules.length == 0 ||
									editingEmissionRules.some((ec) => isEmissionRuleEmpty(ec))
								}
						...
						{!!emissions?.state.hasRuleBasedEmissions && (
							<div className="d-flex align-items-center gap-1 text-ocean-green">
								<FaCheck />
								<Qty
									value={emissions.overallRuleBasedEmissions}
									unit={emissions.unit}
								/>
							</div>
						)}
						...
						<Card>
							<Card.Body>
								<Stack direction="horizontal">
									...
									<div>
										<Qty
											value={emissions.overallImportedEmissions}
											unit={emissions.unit}
										/>
									</div>
								</Stack>
			...
			{!!emissions?.state.hasEmissions && (
				<Row>
					...
					<Col xl={6} className="mb-3">
						<Heading>
							<FaDiagramProject /> Object Allocation
						</Heading>
