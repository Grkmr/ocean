...
const OceanApp: React.FC<AppProps<InitialPageProps>> = ({
	Component,
	pageProps,
}) => {
	const pageName =
	...
	const { errorMessage, setErrorMessage } =
		useOceanStore.useState.errorMessage();
	...
	const { objectEmissionResults, setObjectEmissionResults } =
		useOceanStore.useState.objectEmissionResults();
	...
	const { emissionRules, setEmissionRules } =
		useOceanStore.useState.emissionRules();
	 ...
	 */
	const { climatiqUnits, setClimatiqUnits } =
		useOceanStore.useState.climatiqUnits();
	...
	useInitEffect(async () => {
		...
		try {
			...
			if (!unitTypes) {
				setErrorMessage(
					"Lost connection to the server. Try to restart the session.",
				);
			} else {
	...
	const apiWrapper = async <F extends ApiF>(
		...
		const { skipMutex = false, loadingText, onApiError } = options;
		...
		switch (task.taskState) {
			...
			case "RETRY":
				...
				scheduleTaskPoll<F>(
					...
					> as unknown as ApiWrapper<typeof Api.taskStatusTaskStatusGet>,
					tasks,
					task,
					newSession,
					options,
	...
	const processApiResponse = async <F extends ApiF>(
		...
		if (isTask && "task" in responseData) {
			...
			const { finish, responseData: newResponseData } =
				await processTaskResponse(task, responseData, options);
		...
		if ("emissions" in responseData && !!responseData.emissions) {
			setEmissions({
				...
				apiState: isComputeEmissions
					? responseData.state
					: (emissions?.apiState ?? "old"),
			});
		}
		if ("objectEmissions" in responseData) {
			if (!("objectAllocationConfig" in responseData))
				throw Error(
					"Response contains object emissions, but no allocation config",
				);
		...
		if (isImportOrLoad) {
			if (
				...
				!("ocel" in responseData) ||
				!responseData.ocel
			) {
				throw Error(
					"Cannot import app state - session or OCEL undefined in response",
				);
			}
			const appState = responseData.appState ?? options.importedAppState;
			// setPreventUpdates(1)
			await applyImportedAppState(
				(appState ?? {}) as AppStateExport,
				responseData.session,
				responseData.ocel as OCEL,
				apiWrapper,
			);
	...
	return (
		<QueryClientProvider client={queryClient}>
			<ConfirmationModalProvider>
				<Layout pageName={pageName} darkMode={darkMode}>
					<Component
						isClient={isClient}
						darkMode={darkMode}
